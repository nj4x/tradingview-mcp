[liteparse] extract: 82.7ms (17 pages)
[liteparse] ocr render: 23.2ms (1 pages)
[liteparse] ocr: 3950.7ms
[liteparse] project: 8.8ms
[liteparse] total: 4066.4ms
...
Toolformer: Language Models Can Teach Themselves to Use Tools
    ...
    Maria Lomeli Luke Zettlemoyer Nicola Cancedda Thomas Scialom
     ...
     abilities to solve new tasks from just a few          Medical Society] the MMS.
    ...
    best incorporate the results into future token         California State Legislature that guarantees the public's
     prediction. This is done in a self-supervised         right to attend and participate in meetings of local
     way, requiring nothing more than a handful of         legislative bodies.] that requires legislative bodies, like
      demonstrations for each API. We incorporate          city councils, to hold their meetings open to the public.
...
LM Dataset                  1        2                                —                 3        LM Dataset
                            Sample API Calls      Execute API Calls                     Filter API Calls      with API Calls
                                                   ...
                                                   1    1                               1 → Steel City)
    x1:i-1 = Pittsburgh is  = What other name is  ci    ri   = Steel City           Li(ci        x* = Pittsburgh is
    also known as               Pittsburgh known by?                                < min(Li(ci1 → ε), Li(ε))     also known as
    xi:n = the Steel City   = Which country is   ci2  r 2 = United States                                         [QA(What …?
                                i                                                   Li(ci2 → United States)       → Steel City)]
    ...
    sample a position i and corresponding API call candidates c       1, c2  , . . ., ck.     We then execute these API calls and
                                                                      i     i       i
    ...
    finetune the LM itself on the API calls that it con-          inserted into text sequences are shown in Figure 1.
                                                                                        1
    siders useful. As illustrated in Figure 1, through                Given a dataset C = {x , . . . , x            |C|} of plain
    ...
    learning to use tools, Toolformer, which is based                 1     In practice, we use the token sequences " [", "]" and
...
information required to complete the text. You can call the         sponding results. How this is done depends entirely
  ...
  question you want to ask. Here are some examples of API           ing another neural network, executing a Python
    ...
    Input: Joe Biden was born in Scranton, Pennsylvania.            over a large corpus. The response for each API call
    ...
    Output: Coca-Cola, or [QA("What other name is                   Li(z) = - Xn wj-i · log pM(xj | z, x1:j-1)
  Coca-Cola known by?")] Coke, is a carbonated soft drink                     j=i
    ...
    Output:                                                          +
                                                                   L i   = Li(e(ci, ri))
    Figure 3: An exemplary prompt P(x) used to generate            L -   = min (Li(ε), Li(e(ci, ε)))
    API calls for the question answering tool.                       i
    ...
    tions for doing API calls by computing, for each                          L  i     - L i     ≥ τf
    i ∈ {1, . . . , n}, the probability                             holds, i.e., adding the API call and its result reduces
               pi = pM(<API> | P(x), x1:i-1)                        the loss by at least τf , compared to not doing any
    ...
    API calls c 1                                                                                                                 ∗ =
        i    , . . . , cm          by sampling from M given the     position i, we construct the new sequence x
        i
    sequence [P(x), x1, . . . , xi-1, <API>] as a prefix            3We provide e(ci, ri) as a prefix instead of inserting it at
...
exposes it to the same content as finetuning on C.            Machine Translation System Our fourth tool is
...
Number of Examples                                                                        • GPT-J: A regular GPT-J model without any
    ...
    Machine Translation             3,156       1,034    229              • Toolformer: GPT-J finetuned on C∗             , our sub-
    Table 2: Number of examples with API calls in C∗ for                       set of CCNet augmented with API calls.
    different values of our filtering threshold τf.                                       • Toolformer (disabled): The same model as
    ...
    wt = P     w˜  t      with w˜      max(0, 1 - 0.2 · t)                els, respectively.
        s∈N w˜    s                   t =                                 4.2  Downstream Tasks
...
crease the disposition of our model to make use of         Toolformer (disabled)     22.1            6.3     34.9
                                                           Toolformer                33.8           11.5     53.5
the APIs that it has access to. At the same time,          OPT (66B)                 21.6            2.9     30.1
...
in a left-to-right fashion. To account for different      calculator tool for most examples, clearly outperform-
tokenizations and added complexity from not in-           ing even OPT (66B) and GPT-3 (175B).
...
4.2.2  Math Datasets                                      swering datasets considered by Brown et al. (2020).
...
Results are shown in Table 5.                 Once again,     that are given contexts and questions in English.
...
4.2.4    Multilingual Question Answering                      et al., 2020). As an upper bound, we also evaluate
...
given that named entities in TEMPLAMA are often            els from the GPT-2 family (Radford et al., 2019),
...
ity et al., 2017) and a subset of 10,000 randomly          Decoding Strategy We investigate the effect of
selected documents from CCNet (Wenzek et al.,              our modified decoding strategy introduced in Sec-
2020) that were not used during training. Perplex-         tion 4.2, where instead of always generating the
...
calls for more examples - from 40.3% and 8.5%               10     53.5 54.0 22.5 98.1         26.3 26.4     -        100.0
...
model is calibrated to some extent: It decides to            examples for which the model decides to call an API
...
CCNet augmented with API calls, as well as the               Language Model Pretraining There are various
                      -      +
corresponding score L i  - L i that is used as a fil-        approaches that augment language models with
...
the model are intuitively useful in the given context.       pretraining, including various forms of metadata
                                -      +
As can be seen, high values of Li  - L i       typically     (Keskar et al., 2019), HTML tags (Aghajanyan
...
Table 10: Examples of API calls for different tools, sorted by the value of L- - L+
        i     i     that is used as a filtering
...
7 Limitations
While our approach enables LMs to learn how to References
use a variety of tools in a self-supervised way, there     Armen Aghajanyan, Dmytro Okhonko, Mike Lewis,
are some clear limitations to what can be achieved         Mandar Joshi, Hu Xu, Gargi Ghosh, and Luke Zettle-
...
for another tool). This is due to the fact that API        Extracting relations from large plain-text collections.
...
use in the finetuning dataset. Our current approach        Ricardo Baeza-Yates, Berthier Ribeiro-Neto, et al.
...
these results or to refine its search query in a simi-     Liang. 2013. Semantic parsing on Freebase from
...
to the calculator API. A potential solution to this        Sifre. 2021. Improving language models by retriev-
problem might be to iteratively apply our approach,        ing from trillions of tokens.
...
and Grave, 2021; Parisi et al., 2022). Finally, when       and Databases, pages 172-183, Berlin, Heidelberg.
deciding whether or not to make an API call, Tool-         Springer Berlin Heidelberg.
...
making an API call.                                        Arvind Neelakantan, Pranav Shyam, Girish Sastry,
...
and translation systems via simple API calls. This         2020. Language models are few-shot learners. In
...
Patrick Lewis, Barlas Oguz, Ruty Rinott, Sebastian˘          Online. Association for Computational Linguistics.
...
arXiv preprint arXiv:1910.07475.                             Patrick Lewis, Anton Bakhtin, Yuxiang Wu, and
  Xi Victoria Lin, Todor Mihaylov, Mikel Artetxe, Tianlu     Alexander Miller. 2019. Language models as knowl-
Wang, Shuohui Chen, Daniel Simig, Myle Ott, Na-              edge bases?  In Proceedings of the 2019 Confer-
man Goyal, Shruti Bhosale, Jingfei Du, Ramakanth             ence on Empirical Methods in Natural Language
Pasunuru, Sam Shleifer, Punit Singh Koura, Vishrav           Processing and the 9th International Joint Confer-
Chaudhary, Brian O'Horo, Jeff Wang, Luke Zettle-             ence on Natural Language Processing (EMNLP-
moyer, Zornitsa Kozareva, Mona Diab, Veselin Stoy-           IJCNLP), pages 2463-2473, Hong Kong, China. As-
anov, and Xian Li. 2021. Few-shot learning with              sociation for Computational Linguistics.
...
6951, Online and Punta Cana, Dominican Republic.           David Yarowsky. 1995. Unsupervised word sense dis-
Association for Computational Linguistics.                  ambiguation rivaling supervised methods. In 33rd
...
Goyal, Arthur Szlam, Y-Lan Boureau, Melanie Kam-           Eric Zelikman, Yuhuai Wu, Jesse Mu, and Noah D.
badur, and Jason Weston. 2022. Blenderbot 3: a de-          Goodman. 2022.  Star: Bootstrapping reasoning
ployed conversational agent that continually learns         with reasoning.
...
When sampling and filtering API calls, by default               English. Since most of the CCNet dataset is in
we use values of τs = 0.05 and τf = 1.0 - i.e.,                 English, we filter out the parts that contain only
we only make API calls at positions where the                   English text before generating API calls. More
probability of the <API> token is at least 5%, and              specifically, we only keep those paragraphs which
we keep API calls if they reduce the loss by at least           contain text chunks in a language other than En-
1.0. We only keep the top k = 5 such positions and              glish preceded and followed by English text. We
sample up to m = 5 API calls for each position                  use text chunks of size 10 tokens. To determine
identified in a piece of text. Due to the heuristic             whether the middle text chunk is in a language
filtering described below, we generate API calls for            different than English we again use the fastText
the calculator and machine translation system on                classifier with a confidence greater than 0.8. We
only a small subset of C; to compensate for this,               also filter out any text chunks that contain only
we set τs = 0.0, k = 20 and m = 10 for these                    numbers or special symbols. This filtering mecha-
tools. As the resulting sets of API calls are still             nism allows us to generate data more efficiently by
comparably small, we additionally set τf = 0.5.                 focusing our API call generations in places where
...
keep a random subset of 1%.                                     [QA("In which state is Scranton?")]
                                                                Pennsylvania.
Calendar For creating our dataset C∗           , we operate     Input: Coca-Cola, or Coke, is a
...
18% of the documents.                                           Coca-Cola Company.
                                                                Input: x
Machine Translation For both training and in-                   Output:
ference, we use the 600M parameter NLLB (Costa-                 Calculator  We use the following prompt for the
...
Input: A total of 252 qualifying matches         prompt for the machine translation tool:
...
Wikipedia Search We use the following prompt     Output: [MT(南京高淳县住房和城乡建设局 城市新
                                                 区 设 计)]      a plane of reference Gaochun is
...
Input: The president of the United                     Template                                            Size
...
Input: The number of days from now until               days?
...
Input: The store is never open on the                  year} {is, was} it {the day before yesterday, yes-
weekend, so today it is closed.                        terday, today, tomorrow, the day after tomorrow}?
...
Input: x                                               was, are there until} holiday this year?
Output:                                                Total                                               9,400
B Toolformer Training                                   Table 11: Templates used to create DATESET where
...
els are trained using DeepSpeed's ZeRO-3 (Rasley        erated and used to fill each template, if relevant. The
...
For both LAMA and TEMPLAMA, given an input              DATESET is created by first randomly selecting 500
text x, we use the following prompt: Please             "current dates". For each current date, another rela-
complete the following text so
that it is factually correct:     x.                    tively past/future date is randomly selected within
...
For all math benchmarks, given a context x and          such query using the first template would be, "How
a question q, our prompt is: x q The answer             many days ago was August 14, 2020?" If called,
is.                                                     the Calendar tool would return the presumed cur-
